...
$app->post(
  ...
  function(Request $request, Response $response) use ($app)
  {
    ...
    if ($validated_country && $validated_detail)
    {
      ...
      echo"validated_detail= $validated_detail<br>";
    ...
    else
    {
      $selected_detail = DETAIL_TYPES[$validated_detail];
          ...
          if (isset($country_detail_result['CODE']))
          {
              $comment = 'The Country ID is ' . $country_detail_result['CODE'];
      }

    if (isset($country_detail_result['CURRENCY']) && $validated_detail == "currency")
    {
      $comment = 'The currency in use is the ' . $country_detail_result['CURRENCY'];
    }

      if (isset($country_detail_result['CURRENCYCODE']) && $validated_detail == "curcode") {
